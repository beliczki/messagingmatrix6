...
export default async function Dashboard({
  ...
  const [counts, digest, feeds, freshness, creativeStrip, userRows] =
    await Promise.all([
  ...
  return (
    <div className="dashboard p-8">
      <header className="dashboard__header mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
      ...
      <section className="dashboard__signals mb-6 grid gap-3 md:grid-cols-3">
        ...
        <SignalTile
          ...
          tone={feeds.length === 0 ? "muted" : unpublished > 0 ? "warn" : "ok"}
          href="/feeds"
        />
      ...
      <div className="dashboard__columns mb-6 grid items-start gap-4 xl:grid-cols-[3fr_2fr]">
        ...
        >
          {grouped.length === 0 ? (
            <EmptyLine scope={scope}>Nothing was written in this window.</EmptyLine>
          ) : (
        ...
        <Panel
          title="Feed exports"
          hint={feeds.length > 0 ? `${feeds.length} in this window` : undefined}
          href="/feeds"
        >
          {feeds.length === 0 ? (
            <EmptyLine scope={scope}>No feed was exported in this window.</EmptyLine>
          ) : (
            <ul className="feed-digest divide-y divide-slate-100 text-sm">
              {feeds.map((f) => (
                <li key={f.id} className="feed-digest__row flex items-baseline gap-2 py-2">
                  <Link
                    href={`/feeds/${f.id}`}
...
function groupDigest(
  ...
  const out = new Map<
    string,
    { entityType: string; action: string; n: number; actors: Map<string, number> }
  >();
  for (const r of rows) {
    const key = `${r.entityType}:${r.action}`;
    const cur =
      out.get(key) ??
      { entityType: r.entityType, action: r.action, n: 0, actors: new Map() };
    cur.n += r.n;
...
function DayScopePicker({ scope }: { scope: DayScope }) {
  ...
  return (
    <nav className="day-scope flex items-center gap-1" aria-label="Day scope">
      ...
      <ScopePill href="/" label="Today" active={atToday && scope.range === "day"} />
      <ScopePill
      ...
      />
      <span className="day-scope__date ml-1 font-mono text-xs text-slate-400">
        {scope.date} UTC
      </span>
    </nav>
  );
