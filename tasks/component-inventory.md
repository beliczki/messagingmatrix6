...
- `sankey-view__node--expandable` + `sankey-view__chevron` (6.62.0) — fold-fogantyú: az a szülő, akinek több gyereke van a capnál, **és** a saját `Other`-je is (ugyanaz a toggle). A chevron a `tree-view__chevron` ikonpárja (`ChevronRight`/`ChevronDown`), hogy a két nézet ugyanúgy mondja azt, hogy „van még ez alatt". Expand-állapot: `mm6_sankey_expanded_v1` (a `mm6_tree_expanded_v2` mintájára).
