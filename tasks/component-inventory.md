...
| **DriveHealthCheck** (jobb-toolbar akció) | `drive-health` (`--collapsed`), belül `drive-health__stat`, `drive-health__run` (`toolbar-btn--primary`, fekete), `drive-health__details` (`toolbar-btn`, körvonalas), `__scope`, `__error` | 6.90.0: a panel a VIEW alá került és csak állapot + két gomb; a `__report`/`__line` táblázat a `drive-health-dialog`-ba költözött |
| **PreviewHealth** (jobb-toolbar akció) | `preview-health` (`--collapsed`), belül `preview-health__stat`, `preview-health__run` (`toolbar-btn--primary`, fekete), `preview-health__details` (`toolbar-btn`), `__scope`, `__dot`, `__progress`, `__current`, `__failed`, `__error` | a `drive-health` mintája; a 6.87.0 előtt `creative-library__preview-warning*` volt a *header* toolbarban, listázás-only; 6.90.0: a `__missing*` lista a `preview-health-dialog`-ba került |
