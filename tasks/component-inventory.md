...
- `draft-variants` (`_components`-en kívül, `matrix/DraftVariantSwitcher.tsx`) + `__list` / `__item` / `__add` / `__menu` / `__menu-item` — variánsváltó a szerkesztő fejlécében, a `‹ MC404a › 1/5` léptető mellett. **Szándékosan nem második fülsor** (user, 2026-09-10): a fülök a mátrixszal közösek, a variánsok beléjük hajtása egy felület kedvéért újraírta volna a fülkezelést.
