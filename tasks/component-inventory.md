...
| Comments / History tabok az oldalsávban (6.90.0) | `share-detail-dialog__side-tabs` (`tab-bar` + `tab-bar__tab`), `share-detail-dialog__history` (`-row`, `-action`, `-by`, `-at`, `-fields`, `-loading`, `-empty`, `-error`) | a fejléc-szöveg helyére két tab került; a History a `/share/[id]/history` publikus route-ból tölt |
