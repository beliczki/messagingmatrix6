...
- `drafts-tile__meta` — **két sor, mindig pontosan kettő** (2026-09-10): `__mc` a sajátján, alatta `__sub` = `__product` + `__name`. A korábbi egysoros szabályt a *wrapping* indokolta (egyes kártyákon két sorra nőtt a sor, másokon nem); a fix magasságú kétsoros blokk nem hozza vissza ezt, egyik sor sem tördel, a név csonkol és `title`-ben viszi a teljes szöveget.
- `drafts-tile__menu` + `__menu-btn` / `__menu-list` (globális `dropdown`) / `__menu-item` / `__menu-divider` — ellipszis akció-menü a média jobb felső sarkában: Duplicate as variant · New empty variant · ─ · Archive · Delete. A gomb `opacity-0 group-hover:opacity-100`, nyitott menünél mindig látszik. Kívülre kattintás + Escape zárja (a `creative-library__preview-warning` mintája). A Delete **helyben, két kattintással** erősít, és azt mondja ki, mi lesz a SZÁMMAL — testvér-draft esetén „MC404 stays reserved.", egyébként „free the number?".
- `drafts-tile__matched` — sarok-badge a médián (`status-badge`, `bg-slate-900/80`, `tabular-nums`): hány library-fájl viszi ezt az MC-t, videószámmal. **Balra** ül, mert a jobb felső sarkot az akció-menü kapta.
