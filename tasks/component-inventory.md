...
| `video-thumb__playhead` | Függőleges scrub-pozíció vonal a képen (Frame.io-minta): 1px széles, **szaggatott**, hosszában váltakozó fehér/fekete 50% (`repeating-linear-gradient`, `globals.css`) — így világos és sötét kockán is látszik | VideoThumb | `z-30`, `pointer-events-none`; egérlevételkor eltűnik (a kocka és az idő marad); a `left` az egyetlen valóban számított érték, ezért inline |
...
| **DriveHealthCheck** (jobb-toolbar akció) | `drive-health` (`--collapsed`), belül `drive-health__stat`, `drive-health__details` (`toolbar-btn`, körvonalas), `drive-health__run` (`toolbar-btn--primary`, fekete, ALUL), `__title`, `__scope`, `__error`; gomb-metrika = a mátrix Export gombja (`px-3 py-1.5 text-sm`, `size-4` ikon) | 6.90.0: a panel a VIEW alá került és csak állapot + két gomb; 6.91.1: **dobozos** (`rounded-md border p-3`, mint az `edit-mode-panel` / export-panel), a Details a fekete akció FÖLÖTT; a `__report`/`__line` táblázat a `drive-health-dialog`-ba költözött |
| **PreviewHealth** (jobb-toolbar akció) | `preview-health` (`--collapsed`), belül `preview-health__stat`, `preview-health__run` (`toolbar-btn--primary`, fekete), `preview-health__details` (`toolbar-btn`), `__title`, `__scope`, `__dot`, `__progress`, `__current`, `__failed`, `__error` | a `drive-health` mintája; a 6.87.0 előtt `creative-library__preview-warning*` volt a *header* toolbarban, listázás-only; 6.90.0: a `__missing*` lista a `preview-health-dialog`-ba került; 6.91.1: dobozos, a Details a fekete akció fölött |
| **DriveHealthDialog / PreviewHealthDialog** (6.90.0) | `drive-health-dialog` / `preview-health-dialog` + `__header` (`toolbar`, `h-12`), `__body`, `__hint`, `__table`, `__row`, `__report`, `__line`, `__error`, `__pending-head` | `AppDialog`-ban, a `feed-export-dialog` fejléc-mintájával: cím + elsődleges gomb a fejlécben, tábla a törzsben. A panel nem böngészésre való — ami táblázat, az ide jön |
| **ActivityDigest / ActivityDetailDialog** (dashboard, 6.90.0) | `activity-digest__row--button` (a digest sor kattintható változata), `activity-dialog` + `__header`, `__body`, `__table`, `__row`, `__when`, `__what`, `__id`, `__changed`, `__by`, `__loading`, `__error`, `__more` | a digest-sor mögötti audit-sorok; ugyanaz a dialog-váz, mint a health-dialogoké |
...
- `matrix-agentic-info` (2026-08-17, renamed from `matrix-nondco-info` 2026-09-06) — visual twin of `edit-mode-panel` (`empty-state` tone) shown in the right toolbar's expanded body IN PLACE OF the Edit mode panel when `view === "grid" && axis === "nondco"` (the stored axis token stays `"nondco"`; only the vocabulary is Agentic). `__title` ("Agentic"), `__hint` names the two ways an Agentic MC is born: a correctly-named Creative Library upload, or promoting a draft onto a channel. The Agentic axis disables edit mode entirely (`editApi.editMode` forced off in MatrixGrid).
...
- `share-gallery__image-toggle` (2026-08-31 kiterjesztve) — a kapcsoló kikerült a `ShareGallery`-ből saját modulba (`share/[id]/ImagePreviewToggle.tsx`), mert a **detail-dialog fejléce is ezt használja**. Opcionális `ready`/`total` (a galéria adja, a dialog nem → nincs count-badge) és `compact` (a dialogban csak ikon, felirat nélkül). A mögötte lévő állapot EGY darab: a `ShareGallery` `imagePreview`-ja, amit prop-ként ad le a dialognak — nincs szinkronizáló effect.
- `mode-switch` + `feed-export-dialog__mode` (2026-09-01) — kétállású, leírásos kapcsoló az export-dialog tetején (`Append` / `New feed`). Nem pipa, mert **két különböző cselekvés**, nem egy módosító: az Append meglévő feedet folytat (alap kötelező, a signal és a DEFAULT sor onnan öröklődik), a New feed újat kezd (nincs mihez diffelni, a signalt és a defaultot te választod). Al-osztályok: `__option`, `__option--active`, `__title`, `__desc`.
- `diff-details__filter` (2026-09-01) — szabadszöveges szűrő a Diff details tetején; minden cellaértéken keres, tehát „331" megtalálja az MC-t és „_n2.jpg" a képcserét anélkül, hogy tudni kéne, melyik oszlopban vannak.
- `baseline-picker` (2026-09-01) — egyszeres választó popover a „Compare against"-hez (`matrix/BaselinePicker.tsx`). Natív `<select>` helyett, mert opciónként HÁROM külön olvasandó dolog van: fájlnév (mono), mi ez (`reference` / `export v1`, `· live` ha publikált), és a dátum. A popover-mechanika (kívül-kattintás, Escape, aria-haspopup/listbox) a `MultiPill`-t követi, hogy a kettő egyformán viselkedjen. Al-osztályok: `__button`, `__name`, `__meta`, `__menu`, `__option`, `--empty` (nincs korábbi feed).
...
- `monitoring-table` + `__toolbar` (period-range/platform `select` + match-filter) + `__period-range` / `__period-dash` / `__period-count` (KÉT `select` — a riport-periódus-lista két markere, nem naptár: a `monitoring`-ban nincs nap-dimenzió, egy dátumválasztó nem létező pontosságot ígérne; a `__period-count` csak 1-nél több periódusnál látszik) + `__match-filter` (All/Matched/Unmatched pill-group, a `tab-bar`-szerű inline rounded group) + `__totals` / `__total` (dl grid) + `__wrap` / `__table` (a `users__table` mintát követő `table-auto border-collapse text-sm`)
...
- A szűrők a `monitoring-table__toolbar`-ba kerültek (`toolbar ... sticky top-0 min-h-12 border-b` — a `creative-library__toolbar` mintája): title + Report period from–to select-pár + Platform select (`w-36`, keskeny) + match-pillek + Clear + jobbra compact totál (`toolbar__count`).
...
**W3 size grain + detail dialog:** `monitoring.size` oszlop (migráció `0019`), parser `extractSize` (első `NxN` token a Banner/Adgroupsból), aggregálás size-szinten. Tábla: új sortable **Size** oszlop; minden sor (matched ÉS unmatched) kattintható → **`MonitoringDetailDialog`** (`monitoring-detail`, `modal` shell): matched → `MatrixIframePreview`, unmatched → `status-badge--unmatched` placeholder; mindkettőnél **audience × size bontó tábla** (impr/clicks/CTR + total) az adott MC összes sorából. A `MatrixDetailDialog` használat megszűnt a monitoringban. **W3.i (6.48.0):** a dialógus két táblát fog egy `monitoring-detail__tables` oszlopba — `monitoring-detail__periods` (periódusonkénti impr/klikk/CTR, csak ha a választott tartomány 1-nél több periódust fog át) és a meglévő `monitoring-detail__breakdown`.
...
**Drafts oldal ÚJRAÉPÍTVE — draft = `messages` sor audience nélkül (2026-09-04):**
A régi `draft_messages`-alapú változat blokk-nevei nagyrészt megmaradnak, de a jelentésük változott, és a tartalom-státusz blokkjai (`drafts-tile__status`, `__promoted`) eltűntek — egy draft már nem „renderel/promótált", hanem egyszerűen még nincs cellája.
- `drafts-view` — a lap most **két oszlop**: `__main` (toolbar + `__scroll`) és a jobb oldali `right-toolbar`. Új: `__groups` (briefenkénti szekciók függőleges listája). Megszűnt: `__promoted-toggle`.
- `brief-group` — egy brief és a hozzá tartozó draftok: `__header` (alsó szegélyes, `text-xs uppercase tracking-wider` cím), `__icon`, `__title` (a Slides-linkre mutató `<a>`), `__progress` (`N open · M promoted`, `text-[10px]`), `__empty` ("minden a mátrixban van"). Brief nélküli draftokra a cím szürke „No brief yet".
- `drafts-tile` — most a globális `creative-card` shellre ül rá (`creative-card__thumb` / `__meta` / `__mc`), a Creative Library kártyáival azonos nyelven. Saját elemei: `__media`, `__img`, `__placeholder` („csak sorszáma van"), `__stale` (amber `status-badge` az elavult previewre), `__meta`, `__name`, `__topic` (globális `tag-chip`).
...
**Drafts fal — egy kártya = egy MC (2026-09-10):** a draft már csak a SZÁMOT fogja; a variánsok
ugyanazon a számon lévő `messages` sorok, és a fal egy csempeként olvassa őket.
- `drafts-tile__scrub` / `__scrub-zone` — a médián fekvő N hover-zóna (ahány variáns): vízszintesen áthúzva a cover arra a variánsra vált, a kártyát elhagyva visszaáll `a`-ra. `div`-ek, nem gombok — a kattintást a kártya saját gombja veszi, ezek csak azt terelik, melyik variánsra.
- **Megszűnt:** `drafts-tile__variants` / `__variant` — a betűcsík a preview képről lekerült (user, 2026-09-10). A scrub kiolvasása a `__mc` címke végén ülő betű, ott ahol a szem amúgy is van.
- `drafts-tile__mc` — `MC404`, és **több variáns esetén** a végén az épp mutatott betű (`MC404a` → `MC404b`, ahogy a scrub halad). Egy variánsnál nincs mit váltani, marad `MC404`; `drafts-tile__matched` már **nem** sarok-badge, hanem a meta első sorában szöveg: `matched 18 (a,b)` — a szám az MC-hez tartozik, és hogy mely VARIÁNSOKRA érkezett anyag, azt egy sarok-badge sosem tudta elmondani.
- `draft-variants` (`_components`-en kívül, `matrix/DraftVariantSwitcher.tsx`) + `__list` / `__item` (`--ghost`) / `__add` / `__delete` / `__menu` / `__menu-item` — variánsváltó a szerkesztő fejlécében. `--ghost`: szaggatott, halvány betű, amit az MC **máshol** már visz (a Creative Libraryben MC404b néven leszállított fájlok), de nincs mögötte draft sor — nincs mit szerkeszteni rajta, de a fejléc így mondja el a teljes igazságot arról, mely betűk léteznek. `__delete` a nyitott variánst törli, két kattintással, és a címke kiírja MELYIK betű megy (egy karakteres gombsorban a puszta „Delete" nem mondaná meg), a `‹ MC404a › 1/5` léptető mellett. **Szándékosan nem második fülsor** (user, 2026-09-10): a fülök a mátrixszal közösek, a variánsok beléjük hajtása egy felület kedvéért újraírta volna a fülkezelést.
- `promote-dialog` + `__header` / `__body` / `__field` / `__variants` / `__variant` / `__actions` — a kártya menüjéből nyíló promote-dialógus (`ModalBackdrop` + `modal`, `max-w-md`). **Nincs `<form>`**: a két gomb MAGA a döntés (`Promote N` / `Promote and archive N`), és abban különböznek, mi lesz a bent maradó variánsokkal; egy formban ehhez rejtett mező vagy submitter-vizsgálat kellene, az Enter pedig némán választana egyet.
- **Megszűnt:** `promote-tab__*` (a fül és a fájl is) — a promote a kártya dialógusába költözött.

- `drafts-tile__open` — a csempe kattintható törzse. A külső elem **`div`, nem `button`**: az akció-menü maga is gomb, és gomb a gombban érvénytelen markup (a böngésző lezárja a külsőt, és a kártya saját kattintása elromlik egy részén).
- `drafts-tile__meta` — **két sor, mindig pontosan kettő** (2026-09-10): `__sub` = `__product` + `__mc` az elsőn, `__name` a másodikon. `pr-8`, mert a jobb alsó sarkot az akció-menü foglalja. A korábbi egysoros szabályt a *wrapping* indokolta (egyes kártyákon két sorra nőtt a sor, másokon nem); a fix magasságú kétsoros blokk nem hozza vissza ezt, egyik sor sem tördel, a név csonkol és `title`-ben viszi a teljes szöveget.
- `drafts-tile__menu` + `__menu-btn` / `__menu-list` (globális `dropdown`) / `__menu-item` / `__menu-divider` — vízszintes ellipszis a kártya **jobb alsó** sarkában, **felfelé nyíló** menüvel (`bottom-full mb-1`): a gomb a kártya alsó élén ül egy masonry falban, lefelé nyílva kilógna a kártyából és az utolsó soron a görgetőből is: Duplicate as variant · New empty variant · ─ · Archive · Delete. A gomb `opacity-0 group-hover:opacity-100`, nyitott menünél mindig látszik. Kívülre kattintás + Escape zárja (a `creative-library__preview-warning` mintája). A Delete **helyben, két kattintással** erősít, és azt mondja ki, mi lesz a SZÁMMAL — testvér-draft esetén „MC404 stays reserved.", egyébként „free the number?".
- `drafts-tile__matched` — sarok-badge a médián (`status-badge`, `bg-slate-900/80`, `tabular-nums`): hány library-fájl viszi ezt az MC-t, videószámmal. Bal felül ül.
- `brief-tab__target` a Brief fülön most **„Production target"** néven fut, és **alatta** ül a planned topic — mert a target dönti el, milyen alakot vesz fel.
- `planned-topic` + `__row` (a négy rész EGY sorban, kulcs-sorrendben) / `__tag` / `__tag4` / `__preview` (`matrix/PlannedTopicField.tsx`) — a tervezett topic szerkesztője. **DCO / Both**: `tag1`–`tag3` legördülő a dimenzióban ténylegesen használt szótárból (`NA` elöl, mert az a leggyakrabban helyes válasz a 2–3. tagre), plusz `tag4` szabad szöveg; a négy rész `{{product}}_{{tag1}}_{{tag2}}_{{tag3}}_{{tag4}}` alakban áll össze, és a `__preview` sor kiírja, mi lett belőle. **Agentic**: egyetlen szabad mező, mert azok a topicok fájlnevekből szintetizált stringek, nincs mögöttük topics sor, amiből szótár jönne. A mérvadó generátor az `entities/topics.ts` `keyFromPattern`-je; ez `_`-ra fűz és `_`-nál bont, hogy a két fél egymással konzisztens legyen.
- `brief-tab__intake-row` — Draft name + Product egy sorban (a név `flex-1`, a product `w-36 shrink-0`), a kártya olvasási sorrendjében. A Brief fül `Field`-jei **hint nélkül** állnak (user, 2026-09-10): a Brief slide inputnak placeholdere van, a hibát pedig a mező alatti `form-field__error` mondja ki.
- **Megszűnt:** `brief-tab__variant` (a két variáns-gomb a kártya akció-menüjébe költözött) és a Promote fül `promote-tab__discard` párosa (Archive/Delete ugyanoda). A Promote fül egy kérdést válaszol meg: hova.
...
- `matrix-grid__x--edge-r` (border-right-color) / `--edge-b` (border-bottom-color) — hoverkor a cellához tartozó oszlop (bal+jobb) és sor (alsó+felső) él-vonala `--mx-cross` színt kap. Imperatívan toggle-ölve (`GridView.paintCrosshair`, nincs state/re-render); `data-col-key`/`data-row-key` a headereken + cellákon. Csak border-COLOR vált (0 layout-shift), `transition: border-color 140ms`. globals.css VÉGÉN, rétegen kívül. Token: `--mx-cross` — semleges szürke (light `rgba(0,0,0,0.4)` / dark `rgba(255,255,255,0.4)`), NEM brand-szín (6.25.0).

**Sidebar theme toggle + verzió (2026-08-25, 6.25.0, Confai2-minta):**
- `app-sidebar__theme` — a footer aljára pinnelt theme+verzió blokk. Expanded: `app-sidebar__theme-pill` (két `app-sidebar__theme-btn` Sun/Moon, aktív = `bg-brand-button`) + inline `app-sidebar__version` (font-mono `text-[10px]`). Collapsed: függőleges `app-sidebar__version` (`-rotate-90` egy `h-11` box-ban) + `app-sidebar__theme-round` (kör-ikon gomb). A toggle a `.dark` class-t váltja + `localStorage.mm6_theme`-et ír (per-böngésző, light/dark only). A verzió a nav-ból ide költözött; a `DesignTab` `color-mode-field` (light/dark/**system** pill) MEGSZŰNT.
...
- `matrix-agentic-info` már fent — az Agentic tengely edit-panel helyettesítője.
...
| Mobil akció-menü (2026-09-04) | `share-actions-menu` + `__trigger` / `__panel` / `__section` / `__section-label` / `__row` (`share/[id]/ShareActionsMenu.tsx`) | `sm:hidden`; a jobb oldali gombsor (Download all · Drive · nézet · image preview · téma) egyetlen „Actions" lenyílóba tömörítve, szekció-címkékkel és teljes szélességű, feliratos sorokkal. Desktopon a régi gombsor él (`sm:flex`). |
...
| Drive-mappa a detail-dialog fejlécében | `share-detail-dialog__drive` (+ `toolbar-btn`) | 2026-09-04: aláhúzott szövegből **borderes gomb** lett a Download mellett, ugyanazzal az outline Drive-ikonnal, mint a galéria; 6.90.0-tól a felirata csak `Drive` |
| Brief-deck gomb a vezérlő sávban (6.90.0) | `share-gallery__slides-btn` — több decknél `share-gallery__slides` (wrapper) + `__slides-count` + `__slides-menu` / `__slides-menu-item` | a Drive-gomb ikertestvére, ugyanazzal a szerkezettel; `slides` ikon (lucide `Presentation`). Feliratok párban: `Drive` / `Slides` |
| Brief-deck a detail-dialog fejlécében (6.90.0) | `share-detail-dialog__slides` (+ `toolbar-btn`) | a megnyitott kártya saját briefje (MC szerint), a Drive-gomb mellett |
| Comments / History tabok az oldalsávban (6.90.0) | `share-detail-dialog__side-tabs` (`tab-bar` + `tab-bar__tab`), `share-detail-dialog__history` (`-section`, `-label`, `-row`, `-action`, `-by`, `-at`, `-fields`, `-loading`, `-empty`, `-error`) | a fejléc-szöveg helyére két tab került; a History a `/share/[id]/history` publikus route-ból tölt. 6.91.0: **szekciókra bontva** (kártya vs. kreatív fájl), `__history-section` vonallal, `__history-label` szürke sáv-fejléccel |
...
- ~~`feed-export-dialog__rows`~~ (6.60.0-ban bevezetve, **6.61.1-ben törölve**) — a tábla a dialóguson belül szűkítette az export saját setupját, és a dialógus nem sorböngészésre való, hanem a döntésre, hogy mi megy ki. A preview kint él, a panel `Preview feed rows` checkboxán, és csak ott.
- `feed-export-panel__preview-toggle` + `feed-preview-dialog` (`__header`, `__hint`, `__body`) (6.61.0) — „Preview feed rows" checkbox **közvetlenül az Export gomb fölött** a toolbar feed-ágában; `AppDialog`-ban nyitja a `FeedView`-t a jelenlegi szűrésre. Külön a `feed-export-dialog__rows`-tól: az a diff mellett azt mondja meg, *mit küld ez az export*; ez azt, *mi van most a feedben*.
- `sankey-view__node--expandable` + `sankey-view__chevron` (6.62.0, 6.63.0-ban egyszerusitve) — fold-fogantyu az `Other` node-on: kattintasra az **egesz oszlop** kinyilik. A chevron a `tree-view__chevron` `ChevronRight`-ja, hogy a ket nezet ugyanugy mondja azt, hogy van meg. Expand-allapot: `mm6_sankey_expanded_v2` (oszlop-indexek, a `mm6_tree_expanded_v2` mintajara).
- `sankey-metric-panel` (6.64.0) — a jobb toolbar `Weight by` doboza sankey nezetben, az `edit-mode-panel` / `export-panel` vizualis ikertestvere. `__title`, `__switch` = `toggle-group` + `ToggleBtn` (MC / Impr. / Cost), `__period` = `form-field` select a riport-idoszakra, `__hint` = magyarazo sor MC modban, `__coverage` (+ `--low`, amber `AlertTriangle`-lel 70% alatt) = mennyi kotheto uzenethez. Perzisztencia: `mm6_sankey_metric`, `mm6_sankey_period`.
...
| **ActivitySparkline** (dashboard, 6.108.0) | `activity-sparkline` + `__svg`, `__base`, `__area`, `__line`, `__peak`, `__hit`, `__axis`, `__peak-label` | szerver-renderelt inline SVG az Activity panel tetején; egy széria → nincs legend, a panelcím nevezi meg. Minden szín `currentColor`-ból (a dark-shim a `stroke-*`-ot nem fedi) |
| **PreviewPane verzió-választó** (6.109.0) | `preview-pane__version` (a `rightExtras` slotban) | csak akkor renderel, ha az adott mérethez több leszállított verzió van; `n1` / `n2 · latest`, alapértelmezés a legmagasabb. A létrát a `versionLadder` adja, ugyanaz, amivel a drafts-kártya borítója választ |

| **ApiTab** (Settings › API, 6.110.0) | `api-tab` + `__section`, `__section-title`, `__section-body`, `__def`, `__group`, `__group-title`, `__routes`, `__recipe`, `__secret`; útvonalanként `api-route` (`__header`, `__methods`, `__path`, `__description`) | a `mcp-tab__*` család ikertestvére, mert ugyanaz a dolga: generált inventár + kézi próza. Az auth-jelvény a globális `status-badge`-re ül, nem új családot nyit. A `__recipe` a HMAC-egysoros `<pre>`-je, a `__secret` a felfedés/rotálás doboza (`modal` + `token-reveal__box` újrahasználva a McpTab-ból) |
