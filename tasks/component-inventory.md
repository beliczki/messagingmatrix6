...
| **DriveHealthCheck** (jobb-toolbar akció) | `drive-health` (`--collapsed`), belül `drive-health__stat`, `drive-health__run` (`toolbar-btn--primary`, fekete), `drive-health__details` (`toolbar-btn`, körvonalas), `__title`, `__scope`, `__error` | 6.90.0: a panel a VIEW alá került és csak állapot + két gomb; 6.91.1: **dobozos** (`rounded-md border p-3`, mint az `edit-mode-panel` / export-panel), a Details a fekete akció FÖLÖTT; a `__report`/`__line` táblázat a `drive-health-dialog`-ba költözött |
