...
export const GET = withSession<Params>(({ claims, params }) => {
  ...
  const row = getAudience(claims.cid, id);
  if (!row) return NextResponse.json({ error: "not_found" }, { status: 404 });
  return NextResponse.json({ audience: row });
});
...
export const PATCH = withSession<Params>(async ({ req, claims, params }) => {
  ...
  const result = updateAudience(claims.cid, id, expected, input);
  if (!result.ok) {
    if (!result.current) {
  ...
  return NextResponse.json({ audience: result.row });
});
