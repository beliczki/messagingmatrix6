...
// last tick already sits within this many seconds of the end, so the strip does
// not finish on two near-identical frames.
export const END_FRAME_MIN_GAP_SEC = 1;
...
export const STILLS_CACHE_VERSION = 3;
...
export function wantsEndFrame(durationSec: number, tickCount: number): boolean {
  ...
  const lastTick = FIRST_STILL_OFFSET_SEC + (tickCount - 1) * STILL_INTERVAL_SEC;
  return durationSec - lastTick >= END_FRAME_MIN_GAP_SEC;
}
